...
export async function hostRoutes(server: FastifyInstance) {
  ...
  server.post('/', async (request, reply) => {
    ...
    if (hosts.some((entry) => entry.name === host.name)) return reply.status(409).send({ error: 'Host already exists' })
  ...
  server.put<{ Params: { name: string } }>('/:name', async (request, reply) => {
    ...
    if (oldName !== newHost.name) {
      ...
      await writePrograms(programs.map((program) => (program.host === oldName ? { ...program, host: newHost.name } : program)))

      const allBlocks = await readAllScheduleBlocks()
      const renamed = allBlocks.map((block) => ({
        ...block,
        hosts: block.hosts.map((host) => (host === oldName ? newHost.name : host)),
      }))
