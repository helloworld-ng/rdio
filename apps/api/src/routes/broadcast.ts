...
export function broadcastRoutes(server: FastifyInstance) {
  ...
  server.get("/live.mp3", proxyLiveStream);
}

export function liveStreamRoutes(server: FastifyInstance) {
  server.get("/live.mp3", proxyLiveStream);
}
