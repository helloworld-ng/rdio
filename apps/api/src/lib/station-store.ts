...
export const broadcastStatusFile = path.join(scheduleDirectory, 'broadcast-status.txt')
...
export function mediaItemFromFile(fileName: string, size: number, uploadedAt: Date, contentType?: string): MediaFile {
...
export async function readScheduleBlocksForDay(day: string): Promise<ScheduleBlock[]> {
  try {
...
export function detectBlockConflicts(blocks: ScheduleBlock[]): ScheduleBlockConflict[] {
...
export async function scheduleResponse(station: RadioStation, baseUrl?: string) {
...
export async function nowPlayingResponse(station: RadioStation, baseUrl?: string) {
  const blocks = await readAllScheduleBlocks()
  const at = new Date()
...
export async function initializePlayout(log: { info: (message: string) => void }) {
  ...
  try {
    ...
    if (legacy.length > 0) {
      ...
      log.info(`Migrated ${legacy.length} blocks from blocks.json to daily files`)
    }
