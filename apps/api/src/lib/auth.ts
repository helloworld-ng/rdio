...
export async function requireAdminSession(request: FastifyRequest, reply: FastifyReply) {
  ...
  if (!(typeof session.user.role === 'string' && session.user.role.split(',').includes('admin'))) {
...
export function addAuthGuard(server: FastifyInstance) {
  server.addHook('preHandler', async (request, reply) => {
    ...
    if (session.user.mustChangePassword && pathname !== '/auth/change-password' && pathname !== '/auth/me') {
