...
Edit `apps/api/.env` and add your R2 credentials. See [Cloudflare R2](/docs/cloudflare-r2).
...
To test live broadcast locally, connect BUTT to `localhost:8005` with user `source`, password from `ICECAST_SOURCE_PASSWORD`, and mount `broadcast.mp3`. Schedule a live broadcast slot first. See [Live broadcast](/docs/live-broadcast).
