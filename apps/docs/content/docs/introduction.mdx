...
### 1. The admin UI manages station state
...
### 2. The API chooses the current source
...
### 3. Liquidsoap switches between modes
...
### 4. Icecast serves the public stream
...
Start with the repository README while these docs are being expanded:

- Install dependencies with `pnpm install`.
- Start local infrastructure with `docker compose up`.
- Run the apps with `pnpm dev`.
- Open the admin UI at `http://localhost:5173`.

The docs will grow from here into setup, deployment, scheduling, live broadcast, and operations guides.
