...
import { defineStation, findStation, getStationScheduleSnapshot } from './station'
...
describe('schedule', () => {
  it('finds the current program', () => {
    expect(getCurrentProgram(slots, new Date('2026-05-03T09:00:00.000Z'))?.id).toBe('slot-1')
  })

  it('returns conflicts for overlapping slots', () => {
    const conflicts = detectScheduleConflicts([
      ...slots,
      { ...slots[0], id: 'slot-2', startsAt: '2026-05-03T09:30:00.000Z', endsAt: '2026-05-03T11:00:00.000Z' },
    ])
  ...
  it('builds a schedule API snapshot for a station', () => {
    ...
    const snapshot = getStationScheduleSnapshot(station, new Date('2026-05-03T09:00:00.000Z'))
  ...
  it('finds stations by id or slug', () => {
    const station = defineStation({ id: 'station-1', name: 'Station One', slug: 'one' })
